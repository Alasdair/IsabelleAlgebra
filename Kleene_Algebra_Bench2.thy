...
sledgehammer_params [timeout = 300, provers=e remote_waldmeister spass vampire remote_z3]
...
(* Metis takes a long time here too *)
lemma star_subdist_var_2: "x\<cdot>y \<le> (x+y)\<^sup>\<star>"
  by (metis add_lub distl distr leq_def mult_oner order_refl star_unfoldl)
...
lemma star_denest_var_9: " (x\<^sup>\<star>\<cdot>y\<^sup>\<star>)\<^sup>\<star> = (x\<^sup>\<star>\<cdot>y\<^sup>\<star>)\<^sup>\<star>\<cdot>x\<^sup>\<star>\<cdot>y\<^sup>\<star>"
  by (metis (full_types) add_comm add_lub leq_def mult_assoc mult_double_iso mult_isol mult_oner star_unfoldr)
...
lemma boffa: "x\<cdot>x=x \<longrightarrow> x\<^sup>\<star>=1+x"
  by (metis add_lub distl eq_iff mult_oner order_refl star_inductl star_unfoldl)
