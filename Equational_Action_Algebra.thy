...
by (metis add_lub star_ax)

lemma star_plus_one: "x\<^sup>\<star> = 1+x\<^sup>\<star>"
by (metis add_lub leq_def star_ax)

lemma star_trans: "x\<^sup>\<star>\<cdot>x\<^sup>\<star> \<le> x\<^sup>\<star>"
by (metis add_lub star_ax)

lemma star_trans_eq: "x\<^sup>\<star>\<cdot>x\<^sup>\<star> = x\<^sup>\<star>"
by (smt add_comm add_lub distl leq_def mult_oner star_ax)

lemma star_subid: "x \<le> 1 \<longrightarrow> x\<^sup>\<star> = 1"
by (metis add_lub eq_iff left_galois_counit left_galois_unit left_pure_induction leq_def mult_oner star_ax star_subdist)

lemma star_one: "1\<^sup>\<star> = 1"
by (metis add_lub eq_iff left_galois_counit left_pure_induction mult_oner star_ax)

lemma star_inductl_var: "x\<cdot>y \<le> y \<longrightarrow> x\<^sup>\<star>\<cdot>y \<le> y"
by (metis add_comm add_lub distr left_galois_counit left_galois_unit left_pure_induction leq_def mult_isor mult_onel order_trans star_subdist)
...
by (metis left_galois_counit left_galois_unit left_pure_induction leq_def order_trans star_subdist subdistr)
...
by (smt add_lub iso_subdist mult_isol order_prop order_trans star_ax)

lemma star_rtc1: "1+x+x\<^sup>\<star>\<cdot>x\<^sup>\<star> \<le> x\<^sup>\<star>"
by (metis add_lub star_ax)

lemma star_rtc1_eq: "1+x+x\<^sup>\<star>\<cdot>x\<^sup>\<star> = x\<^sup>\<star>"
by (smt add_assoc add_comm add_idem distl leq_def mult_oner order_refl star_ax)

lemma star_rtc2_eq: "1+x+y\<cdot>y = y \<longrightarrow> x\<^sup>\<star> \<le> y"
by (metis add_assoc add_lub eq_iff left_galois_counit left_galois_unit left_pure_induction leq_def mult_oner order_trans star_subdist subdistl)

lemma star_subdist_var_1: "x \<le> (x+y)\<^sup>\<star>"
by (metis add_lub star_ax)

lemma star_subdist_var_3: "x\<^sup>\<star>\<cdot>y\<^sup>\<star> \<le> (x+y)\<^sup>\<star>"
by (smt add_comm add_lub distl leq_def mult_double_iso mult_oner star_ax star_subdist)
...
by (metis add_lub mult_double_iso order_trans star_ax)

lemma star_zero: "0\<^sup>\<star> = 1"
by (metis add_lub add_zerol eq_iff left_galois_counit left_pure_induction mult_oner star_ax star_subdist)

lemma star_inductr_var_eq: "y\<cdot>x = y \<longrightarrow> y\<cdot>x\<^sup>\<star> \<le> y"
by (metis leq_def mult_isol order_trans right_galois_counit right_galois_unit right_pure_induction star_subdist)

lemma star_inductr_var_eq2: "y\<cdot>x = y \<longrightarrow> y\<cdot>x\<^sup>\<star> = y"
by (metis add_assoc add_idem distl eq_iff leq_def mult_isol mult_oner right_galois_counit right_galois_unit right_pure_induction star_ax star_subdist subdistl)
