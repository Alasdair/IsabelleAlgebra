...
class kleene_algebra = dioid + star_op +
  assumes star_unfoldl: "1\<squnion>x\<cdot>x\<^sup>\<star> \<le> x\<^sup>\<star>"
  and star_unfoldr: "1\<squnion>x\<^sup>\<star>\<cdot>x \<le> x\<^sup>\<star>"
...
lemma star2: "(1\<squnion>x)\<^sup>\<star> = x\<^sup>\<star>"
...
lemma star_unfoldl_eq: "x\<^sup>\<star> = 1\<squnion>x\<cdot>x\<^sup>\<star>"
...
lemma star_unfoldr_eq: "1\<squnion>x\<^sup>\<star>\<cdot>x = x\<^sup>\<star>"
...
begin
  ...
  lemma cstar_unfoldl: "1\<squnion>x\<cdot>(cstar x) = cstar x"
  proof -
    have "is_lub (1\<squnion>x\<cdot>(cstar x)) (powers x)"
      by (metis ex_cstar mult_oner prod_cstar_unique lub_un powers_unfoldl)
    thus ?thesis
      by (metis cstar_unique)
  qed

  lemma cstar_unfoldr: "1\<squnion>(cstar x)\<cdot>x = cstar x"
...
proof
  ...
  show "(x \<squnion> y) \<cdot> z = x \<cdot> z \<squnion> y \<cdot> z" using qdistr1 .
  show "x \<cdot> (y \<squnion> z) = x \<cdot> y \<squnion> x \<cdot> z" using qdistl1 .
...
begin
  ...
  primrec qpower :: "'a \<Rightarrow> nat \<Rightarrow> 'a" where
    "qpower x 0 = 1"
  | "qpower x (Suc n) = x\<cdot>qpower x n"

  definition qpowers :: "'a \<Rightarrow> 'a set" where
    "qpowers x  = {y. (\<exists>i. y = qpower x i)}"

  definition qstar :: "'a \<Rightarrow> 'a" where
    "qstar x \<equiv> \<Sigma> (qpowers x)"

  lemma power_is_qpower: "power x n = qpower x n"
    apply (induct n)
    apply (smt unital_quantale_class.power.simps(1) unital_quantale_class.qpower.simps(1))
    by (metis unital_quantale_class.power.simps(2) unital_quantale_class.qpower.simps(2))

end

lemma powers_is_qpowers: "qpowers x = powers x" by (simp add: qpowers_def powers_def power_def qpower_def)

sublocale unital_quantale \<subseteq> star_continuous_ka
  where zero = bot and cstar = qstar
proof (unfold_locales, clarify)
  fix x y z :: 'a
  show "\<exists>w. is_lub w (dioid.powers_c (1\<Colon>'a) op \<cdot> x y z)" by (metis lub_ex)
  show "x \<cdot> qstar y \<cdot> z = \<Sigma> (dioid.powers_c (1\<Colon>'a) op \<cdot> x y z)"
    apply (simp add: qstar_def powers_def qpowers_def)
    apply (simp add: power_is_qpower)


context unital_quantale
begin

  lemma lub_un_var: "z \<squnion> \<Sigma> A = \<Sigma> ({z} \<union> A)"
    by (metis lub_ex lub_is_lub lub_un)

  lemma qstar_unique: "is_lub w (powers x) \<Longrightarrow> qstar x = w"
    by (metis order_class.lub_is_lub qstar_def)

  lemma lub_predicate_eq: "\<forall>x. P x = Q x \<Longrightarrow> \<Sigma> {y. P y} = \<Sigma> {y. Q y}"
    by (metis (no_types) order_eq_iff predicate1I)

(*
  lemma qstar_unfoldl: "1\<squnion>x\<cdot>(qstar x) = qstar x"
  proof (simp add: qstar_def powers_def)
    have "1 \<squnion> x\<cdot>\<Sigma> {y. \<exists>i. y = power x i} = 1 \<squnion> \<Sigma> {y. \<exists>i. y = x\<cdot>power x i}"
      apply (rule_tac f = "\<lambda>x. 1 \<squnion> x" in arg_cong)
      by (rule_tac f = "\<lambda>x. 1 \<squnion> x" in arg_cong, simp add: inf_distl image_def, rule lub_predicate_eq, blast)
    also have "... = \<Sigma> ({1} \<union> {y. \<exists>i. y = x\<cdot>x\<^bsup>i\<^esup>})" by (metis lub_un_var)
    also have "... = \<Sigma> ({1} \<union> {y. \<exists>i. y = x\<^bsup>Suc i\<^esup>})" by simp
    also have "... = \<Sigma> {y. (\<exists>i. y = x\<^bsup>Suc i\<^esup>) \<or> y = 1}" by (metis Collect_disj_eq Un_commute singleton_conv)
    also have "... = \<Sigma> {y. \<exists>i. y = x\<^bsup>i\<^esup>}"
      by (rule lub_predicate_eq, metis nat.exhaust power.simps(1))
    thus "1 \<squnion> x\<cdot>\<Sigma> {y. \<exists>i. y = x\<^bsup>i\<^esup>} = \<Sigma> {y. \<exists>i. y = x\<^bsup>i\<^esup>}" by (metis calculation)
  qed

  lemma qstar_inductl: "x\<cdot>y \<le> y \<longrightarrow> (qstar x)\<cdot>y \<le> y"
    by (metis power_inductl_var mult_onel powers_c_elim star_lub)
*)

end

sublocale unital_quantale \<subseteq> kleene_algebra where zero = bot and star = qstar
proof
  fix x y z :: 'a
  show "1 + x\<cdot>qstar x \<le> qstar x"


class star_quantale = unital_quantale + star_op +
  assumes qstar: "x\<^sup>\<star> = \<Sigma> (powers x)"

sublocale star_quantale \<subseteq> kleene_algebra where zero = bot sorry (* TODO *)

end

   have "\<Sigma> {y. \<exists>i. y = x\<^bsup>i\<^esup>}"
