...
begin
  ...
  lemma cstar_unfoldl: "one\<squnion>x\<cdot>(cstar x) = cstar x"
      by (metis ex_cstar mult_oner prod_cstar_unique lub_un powers_unfoldl cstar_unique)
...
lemma union_is_join: "X \<union> Y = X \<squnion> Y"
  ...
  by (metis Un_least Un_upper1 Un_upper2 equalityI)

lemma union_lub: "is_lub (X \<union> Y) {X, Y}"
  by (simp add: is_lub_def is_ub_def)

lemma "\<forall>(x::'a lan) y. \<exists>z. is_lub z {x, y}"
  by (metis union_lub)

lemma  "((x::'a lan) \<cdot> y) \<cdot> z = x \<cdot> (y \<cdot> z)"
  by (simp add: l_prod_def, auto, metis, metis append_assoc)

lemma "x \<union> y \<union> z = x \<union> (y \<union> z)"
  by (metis Un_assoc)

lemma " x \<union> y = y \<union> x"
  by (metis Un_commute)

lemma "(x \<union> y) \<cdot> z = x \<cdot> z \<union> y \<cdot> z"
  by (simp add: l_prod_def,  auto)

lemma "(x \<subseteq> y) = (x \<union> y = y)"
  by (metis le_iff_sup)

lemma "(x \<subset> y) = (x \<subseteq> y \<and> \<not> y \<subseteq> x)"
...
lemma "x \<cdot> (y \<union> z) = x \<cdot> y \<union> x \<cdot> z"
  by (simp add: l_prod_def,  auto)

lemma "x \<cdot> {} = {}"
  by (simp add: l_prod_def)

(* now we have proved that languages form a dioid *)

primrec l_power :: "'a lan \<Rightarrow> nat \<Rightarrow> 'a lan"  where
  "l_power x 0 = {[]}"
| "l_power x (Suc n) = x\<cdot>(l_power x n)"

definition star :: "'a lan \<Rightarrow> 'a lan" ("_\<^sup>\<star>" [101] 100)
...
(* Now we have shown that languages form a star continuous kleene algebra *)

lemma set_ext: "\<lbrakk>\<And>x. (x \<in> A) = (x \<in> B)\<rbrakk> \<Longrightarrow> A = B"
by (metis order_eq_iff subsetI)

lemma "(X::'a lan)\<cdot>(\<Union> Y) = \<Union> ((\<lambda>Z. X\<cdot>Z) ` Y)"
  by (rule set_ext, simp add: l_prod_def, auto)

lemma "(\<Union> Y)\<cdot>(X::'a lan) = \<Union> ((\<lambda>Z. Z\<cdot>X) ` Y)"
  by (rule set_ext, simp add: l_prod_def, auto)

(* Now we have shown that languages form a quantale *)

(* To form a unital quantale, just go to the dioid laws *)

(* Languages form a completely distributive quantale *)

lemma "X \<union> \<Inter> Y = \<Inter> ((\<lambda>Z. X\<union>Z) ` Y)"
  by (metis Inter_image_eq sup_Inf)

lemma "X \<inter> \<Union> Y = \<Union> ((\<lambda>Z. X\<inter>Z) ` Y)"
  by (metis Int_Union Union_image_eq)
