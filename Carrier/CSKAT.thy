...
datatype 'a skat_expr = SKLeaf nat "'a trm"
                      ...
                      | SKCon "'a skat_expr" "'a skat_expr" (infixl ":\<parallel>:" 75)
                      | SKStar "'a skat_expr"
                      | SKBool "'a pred bexpr"
                      | SKOne
                      | SKZero
...
| "reads (SKCon x y) = reads x \<union> reads y"
...
| "writes (SKCon x y) = writes x \<union> writes y"
...
| "touches (SKCon x y) = touches x \<union> touches y"
...
| con_compat: "skat_con x1 x2 \<Longrightarrow> skat_con y1 y2 \<Longrightarrow> skat_con (SKCon x1 y1) (SKCon x2 y2)"
...
| assign4: "skat_con (SKBool (bexpr_map (pred_subst x t) \<phi>) :\<odot>: SKLeaf x t) (SKLeaf x t :\<odot>: SKBool \<phi>)"
  ...
  (* Concurrency rules *)
| con_comm: "skat_con (x :\<parallel>: y) (y :\<parallel>: x)"
| exchange: "skat_con ((a :\<parallel>: b) :\<odot>: (c :\<parallel>: d) :\<oplus>: (b :\<odot>: c) :\<parallel>: (a :\<odot>: d)) ((b :\<odot>: c) :\<parallel>: (a :\<odot>: d))"
...
lift_definition con :: "'a::ranked_alphabet skat \<Rightarrow> 'a skat \<Rightarrow> 'a skat" (infixl "\<parallel>" 75) is SKCon
  by (rule con_compat, assumption+)
...
| "skat_unfold (SKCon x y) = skat_unfold x \<parallel> skat_unfold y"
...
| "atoms (SKCon x y) = atoms x \<union> atoms y"
...
next
  show "\<zero> = abs_skat SKZero"
    by (transfer, metis skat_con.refl)
next
  fix s t show "abs_skat s \<parallel> abs_skat t = abs_skat (s :\<parallel>: t)"
    by (transfer, metis skat_con.refl)
qed
...
lemma test_to_pred_expr: "test = pred_expr \<circ> rep_bterm"
  by (metis test_def map_fun_def o_id pred_expr_def)
...
lemma exchange: "(a \<parallel> b) \<cdot> (c \<parallel> d) \<sqsubseteq> (b \<cdot> c) \<parallel> (a \<cdot> d)"
  by (simp add: skat_order_def, transfer, metis exchange)


end

(* +------------------------------------------------------------------------+ *)
subsection {* Kozen's Metatheorem *}
(* +------------------------------------------------------------------------+ *)
...
locale kat_homomorphism =
  ...
  and homo_con: "f (SKCon x y) = f x \<parallel> f y"
...
next
  ...
  assume "(\<And>a. a \<in> atoms p \<Longrightarrow> f a \<cdot> x = x \<cdot> g a) \<Longrightarrow> f p \<cdot> x = x \<cdot> g p"
  and "(\<And>a. a \<in> atoms q \<Longrightarrow> f a \<cdot> x = x \<cdot> g a) \<Longrightarrow> f q \<cdot> x = x \<cdot> g q"
  and "\<And>a. a \<in> atoms (p :\<parallel>: q) \<Longrightarrow> f a \<cdot> x = x \<cdot> g a"
  thus "f (p :\<parallel>: q) \<cdot> x = x \<cdot> g (p :\<parallel>: q)"
    apply (auto simp add: f_homo g_homo kat_homomorphism.homo_con)
