...
sledgehammer
sledgehammer minimize [prover = spass] (c_mult_comm c_mult_isor)
...
subclass independence_algebra_base
proof unfold_locales
qed

end

context independence_algebra_base
...
context independence_algebra_1
...
class independence_semigroup_base = independence_algebra_base + 
...
class independence_semigroup = independence_semigroup_base + independence_algebra_1

context independence_semigroup_base
...
sublocale independence_semigroup \<subseteq> concurrent_semigroup 
...
proof 
  ...
  show "R_prd (S_prd w x) (S_prd y z) \<subseteq> S_prd (R_prd w y) (R_prd x z)"
    by (simp add:subset_eq, auto, simp add:R_prd_elim S_prd_elim, auto, smt agg_assoc agg_comm indep_exchange)
qed
...
subclass independence_algebra_1
proof unfold_locales
qed

end

class independence_monoid = independence_algebra_2 + independence_semigroup +
  fixes U :: "'a set"
  assumes U_unit: "U = {u}"
...
sublocale independence_monoid \<subseteq> concurrent_monoid
...
class dual_independence_algebra_1 = independence_algebra_base +
...
class dual_independence_semigroup = independence_semigroup_base + dual_independence_algebra_1 
...
proof 
  ...
  show "S_prd x y \<subseteq> R_prd x y"
    by (metis R_prd_elim S_sub_R S_prd_elim subsetI)
  show "S_prd x (R_prd y z) \<subseteq> R_prd (S_prd x y) z"
    by (simp add:subset_eq, auto, simp add:R_prd_elim S_prd_elim, smt agg_assoc indep_small_exchange_1_d)
  show  "S_prd (R_prd x y) z \<subseteq> R_prd x (S_prd y z)"
    by (simp add:subset_eq, auto, simp add:R_prd_elim S_prd_elim, metis agg_assoc indep_small_exchange_2_d)
  show "S_prd (R_prd w y) (R_prd x z) \<subseteq> R_prd (S_prd w x) (S_prd y z)"
    by (simp add:subset_eq, auto, simp add:R_prd_elim S_prd_elim, auto, smt agg_assoc agg_comm indep_exchange_d)
qed
